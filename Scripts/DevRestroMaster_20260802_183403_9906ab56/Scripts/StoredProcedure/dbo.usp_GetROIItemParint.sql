...
GO
--DROP  PROCEDURE [dbo].[usp_GetROIItemParint]
CREATE PROCEDURE [dbo].[usp_GetROIItemParint]
@IsMenu bit=NULL
AS
BEGIN
...
inner join ROI_ItemDetails id on c.ITId=id.ITId and c.IsCategory=1 and (c.IsMenu=@IsMenu OR @IsMenu IS NULL) and c.IsArchived=0 --and c.IsActive=1 
and c.PITId=0  
UNION ALL  
...
SELECT categoryorder, categorylevel, CTE.ITId,dbo.fn_LevelPrefix(CTE.CategoryLevel,'----')+cte.ITName ITName, CTE.PITId, CTE.IsActive, CTE.IsMenu, CTE.IsCategory, d.*, cc.* , parent.ITName as  ParentItem  
FROM CTE  
