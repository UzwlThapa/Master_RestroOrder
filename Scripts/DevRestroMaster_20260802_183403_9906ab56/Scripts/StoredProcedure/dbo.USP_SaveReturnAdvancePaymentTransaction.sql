...
GO
CREATE PROCEDURE [dbo].[USP_SaveReturnAdvancePaymentTransaction] 
...
,@Membername NVARCHAR(350)
	...
	, @Membername=CustomerName from Ro_RoomBookings where RoomBookDetailsID=@RoomBookDetailsID 
	...
	DECLARE @TransactionID INT = 0
		SET @VoucherDesc = 'Advance Returned To - ' + @Membername
	...
	SET @TransactionID = @@IDENTITY
		...
		VALUES (
			...
			,@MembershipID
			,case when @PaymentModeID = 1 then 'Cash Paid'
		...
		VALUES (
			...
			,@MembershipID
			,'Advance Returned To - ' + @Membername
			,@PayAmount
			,0
			)
