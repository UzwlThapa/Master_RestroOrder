...
*/
ALTER PROC [dbo].usp_SaveTransactionForSales
(
    @SalesMasterID INT )
AS
    BEGIN
        ...
        SELECT @BillDate = SM.BillDate ,
               @Description = ( 'Sales Bill No :- ' + @code
        ...
        INSERT INTO dbo.Ac_TempTransaction ( TransactionDate ,
                                             ...
                                             PostedOn ,
                                             SalesMasterId )
        VALUES ( @BillDate, @BillDate, @VoucherTypeID, @VoucherNo, @Description, 'System', @BillDate, @SalesMasterID );
        ...
        IF @FoodSales IS NOT NULL
            BEGIN
                SELECT   TOP ( 1 ) @DiscountMethod = CASE WHEN r.isLoyalty = 1 THEN 0
                                                          WHEN r.isLoyalty = 0 THEN r.isflatdis
                                                          ELSE NULL
                                                     END ,
                                   @Discount = CASE WHEN r.isLoyalty = 1 THEN
                                                        CONVERT (DECIMAL (16, 2) ,
                                                                 CASE WHEN r.loyaltydis = '' THEN '0'
                                                                      ELSE r.loyaltydis
                                                                 END)
                                                    WHEN r.isLoyalty = 0 THEN
                                                        CONVERT (DECIMAL (16, 2) ,
                                                                 CASE WHEN r.kotdis = '' THEN '0'
                                                                      ELSE r.kotdis
                                                                 END)
                                                    ELSE NULL
                                               END
                ...
                INSERT INTO dbo.Ac_TempTransactionDetail ( TransactionID ,
                                                           ...
                                                           Debit ,
                                                           Credit )
                VALUES ( @TransactionID, @FoodFcid, NULL, 'Food Sales(KOT)', 0, @FoodSales + @ExtraFoodSales );
            END;
        ...
        FROM   RO_SalesDetailExtra sde
               INNER JOIN RO_SalesDetail sd ON sd.salesDetailId = sde.SalesDetailsId
               LEFT JOIN dbo.CostCenterInfo CC ON sd.CostCenterId = CC.CostCenterId
               LEFT JOIN dbo.RO_CostCenterGroup CCG ON CC.GroupId = CCG.GroupId
        WHERE  sd.salesMasterId = @SalesMasterID
        AND    IsCombo = 0
        ...
        IF @FoodSales IS NOT NULL
            BEGIN
                ...
                INSERT INTO dbo.Ac_TempTransactionDetail ( TransactionID ,
                                                           ...
                                                           Debit ,
                                                           Credit )
                VALUES ( @TransactionID, @BarFcid, NULL, 'BAR Item Sales(BOT)', 0, @FoodSales + @ExtraFoodSales );
            END;
        ...
        IF @FoodSales IS NOT NULL
            BEGIN
                ...
                INSERT INTO dbo.Ac_TempTransactionDetail ( TransactionID ,
                                                           ...
                                                           Debit ,
                                                           Credit )
                VALUES ( @TransactionID, @BakeryFcid, NULL, 'Bakery Cafe Sales  ', 0, @FoodSales + @ExtraFoodSales );
            END;
        ...
        IF @FoodSales IS NOT NULL
            BEGIN
                ...
                INSERT INTO dbo.Ac_TempTransactionDetail ( TransactionID ,
                                                           ...
                                                           Debit ,
                                                           Credit )
                VALUES ( @TransactionID, @OtherSalesFcid, NULL, 'Other Sales  ', 0, @FoodSales + @ExtraFoodSales );
            END;
        ...
        IF @FoodSales IS NOT NULL
            BEGIN
                ...
                INSERT INTO dbo.Ac_TempTransactionDetail ( TransactionID ,
                                                           ...
                                                           Debit ,
                                                           Credit )
                VALUES ( @TransactionID, 33, NULL, 'Combo Sales  ', 0, @FoodSales );
            END;
        ...
        IF @FoodSales IS NOT NULL
            BEGIN
                ...
                INSERT INTO dbo.Ac_TempTransactionDetail ( TransactionID ,
                                                           ...
                                                           Debit ,
                                                           Credit )
                VALUES ( @TransactionID, 22, NULL, 'Room ', 0, @FoodSales );
            END;
        ...
        IF ISNULL (@SalesDiscount, 0) <> 0
            INSERT INTO dbo.Ac_TempTransactionDetail ( TransactionID ,
                                                       ...
                                                       Debit ,
                                                       Credit )
            VALUES ( @TransactionID, 42, NULL, 'Total Discount ', @SalesDiscount, 0 );
        ...
        IF ISNULL (@FoodSales, 0) <> 0
            INSERT INTO dbo.Ac_TempTransactionDetail ( TransactionID ,
                                                       ...
                                                       Debit ,
                                                       Credit )
            VALUES ( @TransactionID, 31, NULL, 'Service Charge ', 0, @FoodSales );
        ...
        IF ISNULL (@FoodSales, 0) <> 0
            INSERT INTO dbo.Ac_TempTransactionDetail ( TransactionID ,
                                                       ...
                                                       Debit ,
                                                       Credit )
            VALUES ( @TransactionID, 44, NULL, 'Other Charges ', 0, @FoodSales );
        ...
        IF ISNULL (@DeliveryCharge, 0) <> 0
            INSERT INTO dbo.Ac_TempTransactionDetail ( TransactionID ,
                                                       ...
                                                       Debit ,
                                                       Credit )
            VALUES ( @TransactionID, 132, NULL, 'Delivery Charges ', 0, @DeliveryCharge );
        ...
        IF ISNULL (@FoodSales, 0) <> 0
            INSERT INTO dbo.Ac_TempTransactionDetail ( TransactionID ,
                                                       ...
                                                       Debit ,
                                                       Credit )
            VALUES ( @TransactionID, 44, NULL, 'Other Discounts ', @FoodSales, 0 );
        ...
        IF ISNULL (@FoodSales, 0) <> 0
            INSERT INTO dbo.Ac_TempTransactionDetail ( TransactionID ,
                                                       ...
                                                       Debit ,
                                                       Credit )
            VALUES ( @TransactionID, 30, NULL, 'VAT ', 0, @FoodSales );
        ...
        WHILE @@FETCH_STATUS = 0
            BEGIN
                ...
                SELECT @PayAmount = PayAmount ,
                       ...
                       @TransactionNo = ( CASE WHEN PaymentModeID = 2 THEN spm.ChequeNo
                                               WHEN PaymentModeID = 3 THEN spm.TransactionNo
                                               WHEN PaymentModeID = 5 THEN spm.TransactionNo
                                               WHEN PaymentModeID = 6 THEN spm.TransactionNo
                                               ELSE NULL
                                          END )
                FROM   RO_SalesPaymentMode spm
                       LEFT JOIN RO_LoyaltyMembership lm ON spm.CusID = lm.MembershipID
                ...
                IF ISNULL (@PayAmount, 0) <> 0
                    BEGIN
                        ...
                        VALUES ( @TransactionID, ISNULL (@FinancialAcID, 15), @MembershipID ,
                                 CASE WHEN @paymentMode = 1 THEN 'Cash Received '
                                      WHEN @paymentMode = 4 THEN 'Credit Party - ' + @CreditParty
                                      WHEN @paymentMode = 5 THEN 'eSewa - ' + @TransactionNo
                                      WHEN @paymentMode = 6 THEN 'fonepay - ' + @TransactionNo
                                      ELSE 'Bank Transaction - ' + @TransactionNo
                                 END, CASE WHEN @paymentMode = 4 THEN @PayAmount --+ @AdvancePayment
                                           ELSE @PayAmount
                                      END, 0 );
                    END;
         ...
         AND @CusID < 0 )
            BEGIN
                ...
                VALUES ( @TransactionID, 43, NULL ,
                         CASE WHEN @NetAmount > @TotalPayAmount THEN 'Deficit Amount Received'
                              ELSE 'Surplus Amount Received '
                         END ,
