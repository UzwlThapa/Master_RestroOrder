...
GO
/****** Object:  StoredProcedure [dbo].[usp_RO_MonthlyClosingTotalSalesStatement]    Script Date: 10/10/2023 10:30:19 AM ******/
SET ANSI_NULLS ON;
GO
...
====================================
 ...
 EXEC dbo.usp_RO_MonthlyClosingTotalSalesStatement '2023-09-27','2023-09-27'

*/
ALTER PROC [dbo].usp_RO_MonthlyClosingTotalSalesStatement
    @startDate DATETIME ,
    @endDate DATETIME
AS
    BEGIN
        ...
        SELECT @Complementry = ( SELECT SUM (Amount) AS Complementry
                                 FROM   dbo.RO_ComplementaryItems
                                 WHERE  CAST(Date AS DATETIME) BETWEEN  dateadd(hour,4, @startDate) AND  dateadd(hour,4, @endDate));

        SELECT @servChrg = SUM (CASE WHEN ba.BilingID = 62 THEN ISNULL (ba.Amount, 0)
                                     ELSE 0
                                END) ,
               @vat = SUM (CASE WHEN ba.BilingID = 54 THEN ISNULL (ba.Amount, 0)
                                ELSE 0
                           END)
        FROM   dbo.RO_BillingAmount ba
               INNER JOIN dbo.RO_SalesMaster sm ON ba.SalesMasterID = sm.salesMasterId
               INNER JOIN dbo.CBMS_BillPostLog bp ON bp.SalesMasterId = sm.salesMasterId
        WHERE  ( sm.BillDate IS NOT NULL
        ...
        FROM     dbo.RO_SalesMaster sm
                 INNER JOIN dbo.CBMS_BillPostLog bp ON bp.SalesMasterId = sm.salesMasterId
                 LEFT JOIN dbo.RO_SalesPaymentMode spm ON spm.salesMasterId = sm.salesMasterId
        ...
        FROM   dbo.RO_SalesMaster sm
               INNER JOIN dbo.CBMS_BillPostLog bp ON bp.SalesMasterId = sm.salesMasterId
        WHERE  ( CAST (sm.BillDate AS DATETIME) BETWEEN  dateadd(hour,4, @startDate) AND  dateadd(hour,4, @endDate))
        AND    ISNULL (sm.IsArchived, 0) = 0
        AND    ISNULL (sm.IsUpdated, 0) = 1;
        ...
        SELECT sm.salesMasterId ,
               CASE WHEN dis.isflatdis = 1 THEN CAST (dis.kotdis AS DECIMAL (18, 2))
                    ELSE CAST (dis.kotdis AS DECIMAL (18, 2)) / 100 * sm.sumKot
               END KOTDiscount ,
               CASE WHEN dis.isflatdis = 1 THEN CAST (dis.bardis AS DECIMAL (18, 2))
                    ELSE CAST (dis.bardis AS DECIMAL (18, 2)) / 100 * sm.sumBev
               END BarDiscount ,
               CASE WHEN dis.isflatdis = 1 THEN CAST (dis.bakerydis AS DECIMAL (18, 2))
                    ELSE CAST (dis.bakerydis AS DECIMAL (18, 2)) / 100 * sm.sumBakery
               END BakeryDiscount ,
               CASE WHEN dis.isflatdis = 1 THEN CAST (dis.pizzadis AS DECIMAL (18, 2))
                    ELSE CAST (dis.pizzadis AS DECIMAL (18, 2)) / 100 * sm.sumPizza
               END PizzaDiscount ,
               CASE WHEN dis.isflatdis = 1 THEN CAST (dis.roomdis AS DECIMAL (18, 2))
                    ELSE CAST (dis.roomdis AS DECIMAL (18, 2)) / 100 * sm.RoomCharge
               END RoomDiscount ,
               CASE WHEN dis.isLoyalty = 1 THEN CAST (sm.totaldiscount AS DECIMAL (18, 2))
                    ELSE 0
               END LoyalityDiscount
        INTO   #temp
        FROM   dbo.RO_SalesMaster sm
               LEFT JOIN dbo.ro_flatandPerDiscount dis ON dis.SalesMasterId = sm.salesMasterId
        WHERE  ( CAST (sm.BillDate AS DATETIME) BETWEEN  dateadd(hour,4, @startDate) AND  dateadd(hour,4, @endDate))
        AND    sm.IsArchived <> 1
        AND    sm.IsUpdated = 1;
        ...
        SELECT   CONVERT (VARCHAR (11), @startDate) + ' / ' + CONVERT (VARCHAR (10), DAY (@endDate)) AS [date] ,
                 sm.salesMasterId ,
                 SUM (ISNULL (( CASE WHEN spm.PaymentModeID = 1 THEN spm.PayAmount
                                     ELSE 0
                                END ) ,
                              0)) CashReceived ,
                 SUM (ISNULL (( CASE WHEN spm.PaymentModeID = 2 THEN spm.PayAmount
                                     ELSE 0
                                END ) ,
                              0)) ChequeReceived ,
                 SUM (ISNULL (( CASE WHEN spm.PaymentModeID = 3 THEN spm.PayAmount
                                     ELSE 0
                                END ) ,
                              0)) CardReceived ,
                 SUM (ISNULL (( CASE WHEN spm.PaymentModeID = 4 THEN spm.PayAmount
                                     ELSE 0
                                END ) ,
                              0)) CreditReceived ,
                 SUM (ISNULL (( CASE WHEN spm.PaymentModeID = 5 THEN spm.PayAmount
                                     ELSE 0
                                END ) ,
                              0)) eSewaReceived ,
                 SUM (ISNULL (( CASE WHEN spm.PaymentModeID = 6 THEN spm.PayAmount
                                     ELSE 0
                                END ) ,
                              0)) FonePayReceived ,
        ...
        FROM     dbo.RO_SalesMaster sm
                 INNER JOIN dbo.CBMS_BillPostLog bp ON bp.SalesMasterId = sm.salesMasterId
                 JOIN dbo.RO_SalesPaymentMode spm ON sm.salesMasterId = spm.salesMasterId
        WHERE    ( CAST (sm.BillDate AS DATETIME) BETWEEN  dateadd(hour,4, @startDate) AND  dateadd(hour,4, @endDate))
        AND      ISNULL (sm.IsArchived, 0) = 0
        AND      ISNULL (sm.IsUpdated, 0) = 1
        GROUP BY sm.salesMasterId;
        ...
        SELECT CONVERT (VARCHAR (11), @startDate) + ' / ' + CONVERT (VARCHAR (10), DAY (@endDate)) AS [date] ,
               ...
               ISNULL (CAST (( @netAmount / @totalBills ) AS DECIMAL (18, 2)), 0) AS [SalesPerBill] ,
        ...
        FROM   #salestemp st
               LEFT JOIN #temp tp ON tp.salesMasterId = st.salesMasterId;
    END;
