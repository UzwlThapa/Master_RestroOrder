...
AS
    BEGIN
        ...
        INSERT INTO @tempSales
                    ...
                    WHERE -- CAST(sm.BillDate AS DATE) BETWEEN CAST(@startDate AS DATE) AND CAST(@endDate AS DATE) 
        ...
        WHERE --  CAST(sm.BillDatee AS DATE) BETWEEN CAST(@startDate AS DATE) AND CAST(@endDate AS DATE) 
               sm.BillDate BETWEEN DATEADD (HOUR, 4, @startDate) AND DATEADD (HOUR, 4, @endDate)
        --AND (sm.IsArchived = 0)
        AND    ( @Status = -1
        ...
        WHERE --CAST(t2.BillDate AS DATE) BETWEEN CAST(@startDate AS DATE) AND CAST(@endDate AS DATE) 
               t2.BillDate BETWEEN DATEADD (HOUR, 4, @startDate) AND DATEADD (HOUR, 4, @endDate)
