...
public class OrderPrint
{
    ...
    public string PrintShiftBill(List<OrderDetailClass> orderDetailList, string tableId, DateTime time, string fromTable, string toTable, string userName, string OrderStatus, int orderMasterID, int OrderNo, int? TokenNo, string CustomerName, string Phone)
    {
        string printSuccessful = "";
        RestrOrderController rocobj = new RestrOrderController();
        List<costCenter> coc = rocobj.getcostcenter();
        ...
        foreach (costCenter cc in coc)
        {
            ...
            if (order != null && order.Count > 0)
            {
                ...
                kot.KOTItems = order;
                kot.CostCenterTitle = cc.CostCenterName + " Order Shift";
                try
                {
                    ...
                    for (int i = 0; i < kotcount; i++)
                    {
                        if (DBPrint)
                        {
                            print.PrintShiftBill(cc.DefaultPrinter, kot, fromTable,toTable);
                        }
                        else
                        {
                            print.PrintShiftBill(printer, kot, fromTable, toTable);
                        }
                    }
                }
                catch (Exception)
