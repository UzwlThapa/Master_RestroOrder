...
public partial class Controls_Sidebar : BaseAdministrationUserControl
{
    ...
    public void BuildCollapsedSidebar(string adminSidebarPosition)
    {
        ...
        foreach (Sidebar parent in lstSidebar)
        {
            ...
            {
                sb.Append("<li class='Grandparent sfLevel0'><a href='" + url + "'>" + GetImage(parent.ImagePath, parent.DisplayName.Replace(" ", "-").ToLower()) + sidebartext + "</a>");
                ...
                foreach (Sidebar child in lstSidebar)
                {
                    ...
                    {
                        sb.Append("<li class='sfLevel0'><a href='" + Childurl + "'>" + GetImage(child.ImagePath, child.DisplayName.Replace(" ", "-").ToLower()) + sidebarchildtext + "</a></li>");
                    }
                    else if (child.ParentID == parent.SidebarItemID && child.ChildCount > 0)
                    {
                        sb.Append("<li class='parent sfLevel0'><a href='" + Childurl + "'>" + GetImage(child.ImagePath, child.DisplayName.Replace(" ", "-").ToLower()) + sidebarchildtext + "</a>");
