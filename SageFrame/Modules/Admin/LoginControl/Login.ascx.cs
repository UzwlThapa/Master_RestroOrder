...
namespace SageFrame.Modules.Admin.LoginControl
{
    ...
    public partial class Login : BaseAdministrationUserControl
    {
        ...
        public string paymentAfterGenerateBill = ConfigurationManager.AppSettings["PaymentAfterGenerateBill"].ToString();
        ...
        private void Alert(string username)
        {
            ShowMessage(SageMessageTitle.Notification.ToString(), "The User: " + username + " is already LoggedIn from Another Device.PLease LogOut First.", string.Empty, SageMessageType.Alert);
            ...
            try
            {
                ...
                if (token != string.Empty && token != null)
                {
                    ...
                    if (objJson.Count > 0)
                    {
                        ...
                        FirstName = firstname;
                        LastName = lastname;
                        lblAlertMsg.Text = firstname + "Successfully Logged In with FaceBook  and the email Address is " + email;
                    }
                }
            }
            catch (Exception exception)
        ...
        public string LinkedInAuth()
        {
            ...
            try
            {
                ...
                if (oauth_token != null && oauth_verifier != null)
                {
                    ...
                    lblAlertMsg.Text = email + " successfully Login with LinkedIn" + FirstName + "  " + LastName;
                    Session.Remove(SessionKeys.ServiceProvider);
                }
            }
            catch (Exception ex)
        ...
        //    Message.Text = "The value of the HiddenField control is " + HiddenField1.Value + ".";
        //}
        //protected void Page_Init(object sender, EventArgs e)
        //{
        //    HttpRequest request = base.Request;
