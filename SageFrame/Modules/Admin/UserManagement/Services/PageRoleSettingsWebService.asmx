...
public class PageRoleSettingsWebService : AuthenticateService
{
    ...
    [WebMethod]
    public int UpdateCreditLimit(int creditLimit, int PortalID, string userName, int userModuleID, string secureToken, string roleName)
    {
        try
        {
            ...
            if (IsPostAuthenticatedView(PortalID, userModuleID, userName, secureToken))
            {
                RoleController objPage = new RoleController();
                objPage.UpdateCreditLimit(creditLimit, roleName.Trim().ToLower());
