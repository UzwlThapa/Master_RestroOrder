...
namespace SageFrame.Modules.Admin.UserManagement
{
    public partial class ctl_ManageUser : BaseAdministrationUserControl
    {
        ...
        {
            user = roc.GetRolesByUsername(GetUsername);
           ...
           private void Includes(){
               IncludeCss("UserManagement", "/Modules/Admin/UserManagement/css/module.css");
        ...
        protected void btnManagePINSave_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtNewPIN.Text != "" && hdnEditUsername.Value != "")
                {
                    if (txtNewPIN.Text.Length == 4)
                    {
                        ...
                        //UserInfo user = new UserInfo(new Guid(hdnEditUserID.Value), Password, PasswordSalt, m.PasswordFormat);
                        //m.ChangePassword(user);
                        int chpin = RestroOrder.RestrOrderController.ChangePIN(hdnEditUserID.Value, txtNewPIN.Text);
                        if (chpin == 1)
                        {
                            ShowMessage("", GetSageMessage("UserManagement", "UserPINChangedSuccessfully"), "", SageMessageType.Success);
                        }
                        else
                        {
                            ShowMessage("", GetSageMessage("UserManagement", "UserPINAlreadyUsed"), "", SageMessageType.Error);
                        }
                    }
                    else
                    {
                        ShowMessage("", GetSageMessage("UserManagement", "PINLength"), "", SageMessageType.Alert);
                    }
                }
                else
                {
                    Response.Write("<script>alert('Empty Pin number')</script>");
                }
        ...
        protected void gdvUser_RowDataBound(object sender, GridViewRowEventArgs e)
        {
                if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                else if (roles.Contains("Site Admin"))
                {
                    if (((LinkButton)e.Row.FindControl("lnkUsername")).Text == "superuser")
                    {
                        e.Row.Visible = false;
                    }
                    else
