(function ($) {
    $.createPageRoleSettings = function (p) {
        ...
        var PageRoleSettings = {
            ...
            UpdateCreditLimit: function (creditLimit, roleName) {
                ...
                $ajaxCall(JSON2.stringify({
                    ...
                    secureToken: SageFrameSecureToken,
                    roleName: roleName
                }), this.config.baseURL + "UpdateCreditLimit", PageRoleSettings.UpdateCreditMessage, function () { });
            },

            UpdateCreditMessage: function (data) {
                //alert(data.d);
                SageFrame.messaging.show(SageFrame.messaging.GetLocalizedMessage("en-US", "PageManager", "CreditLimitUpdateSuccessfully"), "Success");
                $('#editCreditLimit').dialog('close');
            ...
            BindEvent: function () {
                ...
                // Role credit limit edit

                $(".creadit-limit").on("click", function () {
                    $('#editCreditLimit').html('');
                    var rolename = $(this).closest('tr').find('td').eq(0).html();
                    alert(rolename);
                    var html = '';
                    html += '<span> - </span></br>';
                    html += '<input type="text" id="inpCreditLimit"/>';
                    html += '<span></span></br>';
                    html += '<input id="btnUpdateLimit"   data-rolename=' + encodeURIComponent(rolename) +' type="button" value="Update" class="icon-update sfBtn restro-btn restro-btn"/> ';
                    $('#editCreditLimit').append(html);

                    $("#editCreditLimit").dialog({
                        'title': 'Update Credit Limit For "' + rolename +'"',
                        width: 400,
                        modal: true,
                ...
                $('#editCreditLimit').on('keyup paste', '#inpCreditLimit', function () {
                    this.value = this.value.replace(/[^0-9]/g, '');
                });

                $('#editCreditLimit').on('click', '#btnUpdateLimit', function () {
                    var roleName = decodeURIComponent($(this).data('rolename'));
                    var creditLimit = $('#editCreditLimit').find('#inpCreditLimit').val() == '' ? 0 : parseInt($('#editCreditLimit').find('#inpCreditLimit').val());
                    alert(creditLimit);

                    PageRoleSettings.UpdateCreditLimit(creditLimit, roleName);
                });
