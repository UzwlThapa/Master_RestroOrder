(function ($) {
    $.CReport = function (p) {
        ...
        p = $.extend
             ({
                 UserModuleID: '',
                 ModulePath: '/Modules/ClosingReport2/service/',
                 master: '0',
             }, p);
        var v = 0;
        var DiffAmount = 0;
        ...
        var eventFunction = {
            ...
            init: function () {
                $("#btnView").click(function () {
                    ...
                    eventFunction.getClosingReport_StateWise();
                    eventFunction.getClosingReport_CategoryWise();
                    eventFunction.getClosingReport_BillWiseSales();
                ...
                $("#btnPrint").click(function () {
                    ...
                    //if (stat == 0) 
                    {
                        ...
                        eventFunction.print(SalesStatement + StatementStateWise + StatementCategoryWise + StatementBillWiseSales + Statement);
                    }
                    //else {
            ...
            getClosingReport_StateWise: function () {
                var startdate = $("#txtStartDate").val();

                eventFunction.config.method = "getClosingReport_StateWise";
                eventFunction.config.url = eventFunction.config.baseURL + eventFunction.config.method;
                eventFunction.config.data = JSON2.stringify({ startdate: startdate });
                eventFunction.config.ajaxCallMode = 3;
                eventFunction.ajaxCall(eventFunction.config);
            },
            bindClosingReport_StateWise: function (result) {
                var datas = result.d;
                var htmls = "";
                    htmls = '<h3>COSTCENTER WISE</h3>'
                    htmls += '<h4>Date: ' + $("#txtStartDate").val() + '</h4>'
                    htmls += '<table class="tableForClosingReport_StateWise sfGridwrapper nowrap display" cellspacing="0" style="border:none;width:100%;"><thead><tr><th width="600px;">States</th><th>Amount</th></tr></thead><tbody>';
                if (datas.length > 0) {
                    $("#btnPrint").show();
                    $.each(datas, function (index, value) {
                        htmls += '<tr><td style="text-align:center;">' + value.ITName + '</td>';
                        htmls += '<td style="text-align:center;">' + value.Amount + '</td>';
                        htmls += '</tr>';
                    });
                } 
                //else {
                //    htmls += '<tr><td>No Data</td><td></td></tr>';
                //    //$("#btnPrint").hide();
                //}
                htmls += '</tbody></table> <br /><hr /><br />';
                $("#divForClosingReport_StateWise").html(htmls);
                $(".tableForClosingReport_StateWise").dataTable();

                StatementStateWise = htmls;
            },

            getClosingReport_CategoryWise: function () {
                var startdate = $("#txtStartDate").val();

                eventFunction.config.method = "getClosingReport_CategoryWise";
                eventFunction.config.url = eventFunction.config.baseURL + eventFunction.config.method;
                eventFunction.config.data = JSON2.stringify({ startdate: startdate });
                eventFunction.config.ajaxCallMode = 4;
                eventFunction.ajaxCall(eventFunction.config);
            },
            bindClosingReport_CategoryWise: function (result) {
                var datas = result.d;
                var htmls = "";
                    htmls = '<h3>GROUP WISE</h3>'
                    htmls += '<h4>Date: ' + $("#txtStartDate").val() + '</h4>'
                    htmls += '<table class="tableForClosingReport_CategoryWise sfGridwrapper nowrap display" cellspacing="0" style="border:none;width:100%;"><thead><tr><th width="10px">Item</th><th>Unit</th><th>QTY</th><th>Rate</th><th>Amount</th><th>CostCenter</th></tr></thead><tbody>';
                if (datas.length > 0) {
                    $("#btnPrint").show();
                    $.each(datas, function (index, value) {
                        htmls += '<tr><td style="text-align:center;">' + value.ITName + '</td>';
                        htmls += '<td style="text-align:center;">' + value.Symbol + '</td>';
                        htmls += '<td style="text-align:center;">' + value.QTY + '</td>';
                        htmls += '<td style="text-align:center;">' + value.Rate + '</td>';
                        htmls += '<td style="text-align:center;">' + value.QTY * value.Rate + '</td>';
                        htmls += '<td style="text-align:center;">' + value.CostCenterName + '</td>';
                        htmls += '</tr>';
                    });
                }
                else {
                //    htmls += '<tr><td></td><td>No Data</td><td></td><td>No Data</td><td></td><td></td></tr>';
                    $("#btnPrint").hide();
                }
                htmls += '</tbody></table> <br /><hr /><br />';
                $("#divForClosingReport_CategoryWise").html(htmls);
                $(".tableForClosingReport_CategoryWise").dataTable();
                StatementCategoryWise = htmls;
            },
            getClosingReport_BillWiseSales: function () {
                var startdate = $("#txtStartDate").val();

                eventFunction.config.method = "getClosingReport_BillWiseSales";
                eventFunction.config.url = eventFunction.config.baseURL + eventFunction.config.method;
                eventFunction.config.data = JSON2.stringify({ startdate: startdate });
            ...
            bindClosingReport_BillWiseSales: function (result) {
                var datas = result.d;
                var htmls = "";
                    htmls = '<h3>BILL WISE SALES SUMMARY</h3>'
                    htmls += '<h4>Date: ' + $("#txtStartDate").val() + '</h4>'
                    htmls += '<table class="tableForClosingReport_BillWiseSales sfGridwrapper nowrap display" cellspacing="0" style="border:none;width:200px;"><thead><tr><th>S.N.</th><th width="120px">Bill No.</th><th>Basic Amt. </th><th> Disc.</th><th> S.C.</th><th> TaxC.</th><th> Net Amt</th></tr></thead><tbody>';
                if (datas.length > 0) {
                    $("#btnPrint").show();
                    var count = 1;
                    $.each(datas, function (index, value) {
                        htmls += '<tr><td>' + count + '</td>';
                        htmls += '<td>' + value.DATE + '</td>';
                        htmls += '<td class="tot-rig" style="text-align:center;">' + value.TotalAll.toFixed(2) + '</td>';
                        htmls += '<td class="tot-rig" style="text-align:center;">' + value.DISCOUNT.toFixed(2) + '</td>';
                        htmls += '<td class="tot-rig" style="text-align:center;">' + value.ServiceCharge.toFixed(2) + '</td>';
                        htmls += '<td class="tot-rig" style="text-align:center;">' + value.TaxCharge.toFixed(2) + '</td>';
                        htmls += '<td class="tot-rig" style="text-align:center;">' + value.NetAmount.toFixed(2) + '</td>';
                        htmls += '</tr>';
                        count++;
                    });
                }
                //else {
                //    htmls += '<tr><td></td><td>No Data</td><td></td><td></td><td>No Data</td><td></td><td></td></tr>';
                //}
                htmls += '</tbody></table> <br /><hr /><br />';
                $("#divForClosingReport_BillWiseSales").html(htmls);
                $(".tableForClosingReport_BillWiseSales").dataTable();
                StatementBillWiseSales = htmls;
            },
            ...
            bindDataByDates: function (result) {
                ...
                var prev = "";
                var count = "";
                    htmls = '<h3>ITEM WISE</h3>'
                    htmls += '<h4>Date: ' + $("#txtStartDate").val() + '</h4>'
                    htmls += '<table class="tableForMaterizedView sfGridwrapper nowrap display" cellspacing="0" style="border:none;width:100%;"><thead><tr><th width="10px">Item</th><th>Unit</th><th>QTY</th><th>Rate</th><th>Amount</th><th>CostCenter</th></tr></thead><tbody>';
                if (datas.length > 0) {
                    $("#btnPrint").show();
                    $.each(datas, function (index, value) {
                        ...
                        htmls += '<td>' + value.QTY * value.Rate + '</td>';
            ...
            bindStatementDataByDates: function (result) {
                ...
                var datas = result.d;
                var htmls = "";
                    htmls += '<h2>SALES SUMMARIES</h2>';
                    htmls += '<h4>Date: ' + $("#txtStartDate").val() + '</h4>'
                    htmls += '<table class="tableForMaterizedView sfGridwrapper nowrap display" cellspacing="0" style="border:none;width:200px;"><thead><tr><th width="10px">DATE</th><th>No Of Bill</th><th>Basic Amount </th><th> Discount</th><th> Service Charge</th><th> Tax Charge</th><th> Net Amount</th><th> Sales Per Bill</th></tr></thead><tbody>';
                if (datas.length > 0) {
                    $("#btnPrint").show();
                    $.each(datas, function (index, value) {
                    ...
                    SalesStatement += "</div> <br /><hr /><br />";
                    $("#divForStatementData").html(htmls);
                    $(".tableForMaterizedView").dataTable();
                } else {
                    //$("#divForStatementData").html("No Data");
                    $("#btnPrint").hide();
                }
            },
            ...
            ajaxSuccess: function (data) {
                switch (parseInt(eventFunction.config.ajaxCallMode)) {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        eventFunction.bindClosingReport_StateWise(data);
                        break;
                    case 4:
                        eventFunction.bindClosingReport_CategoryWise(data);
                        break;
                    case 5:
                        eventFunction.bindClosingReport_BillWiseSales(data);
                        break;
                }
            },
