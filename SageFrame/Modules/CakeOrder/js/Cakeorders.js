var baseUrl = SageFrameHostURL + "/Modules/CakeOrder/services/CakeOrderWebService.asmx/";
...
function GetItemForSearch() {
    ...
    $.ajax({
        ...
        url: SageFrameHostURL + '/Modules/WholeSale/services/WholeSaleWebService.asmx/GetItemForWholeSaleSearch',
        //data: "",
...
//            $.each(datas, function (index, value) {
//                if (value.LookupName == "cake") {
//                    menuidd = parseInt(value.ItemId);
//                }
//            });
//            GetCategoriesBymenuID(menuidd, "Cake", 1);

//            //var htmls = [];
//            //$('#Menushow').html("");
//            //htmls += "<h4>Cake Items</h4>";
//            //if (datas.length > 0) {
//            //        htmls += "<div class='menus'>";
//            //        $.each(datas, function (index, value) {
//            //            if (value.ImagePath == "")
...
function SaveOrderedData() {
    ...
    if ($('#txtDeliveryService option:selected').val() == 'delivery') {
        var date = $('#txtDeliveryDateTime').val().split('-');
        var timee = date[2].split('T')[1]
        var day = date[2].split('T')[0];       // yields date
        var month = date[1];    // yields month (add one as '.getMonth()' is zero indexed)
        var year = date[0];  // yields year
        var hour = timee.split(":")[0];     // yields hours 
        var minute = timee.split(":")[1]; // yields minutes
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            //    });
            //}
            jAlert('Order saved successfully', "Information!!", function () {
...
function GetDataForSalesBill(orderMasterId) {
    $.ajax({
        ...
        success: function (data) {
            ...
            htmls += "<tr class='disc' style='" + ((orderdetails.length > 0) ? "" : "display:none") + "'><td>KOT ( Rs. " + kotAmount + " ) </td><td>";
            ...
            htmls += "</tr>";
            htmls += "<tr class='disc' style='" + ((orderdetails.length > 0) ? "" : "display:none") + "'><td>Bar ( Rs. " + barAmount + " ) </td><td>";
            ...
            $('#DialogOrderDetail').dialog(
           {
            ...
            $("#selDiscountType").on('change', function () {
                ...
                $('.totle').text('Rs. ' + (totalAmount - roomAmount));
                $('.roomtotle').text('Rs. ' + (totalAmount));
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            {                
                ...
                $('#txtContactNo').val((data.d[0].Phone == null ? "" : data.d[0].Phone));
